...
Embedding::Embedding(int32_t num_cls_tokens, int32_t feature_height,
...
auto Embedding::forward(torch::Tensor x) -> torch::Tensor {
...
auto MultilayerPerceptron::forward(torch::Tensor x) -> torch::Tensor {
  x = layer1->forward(x);
  x = F::gelu(x);
  x = layer2->forward(x);
  x = dropout->forward(x);
...
Block::Block(int32_t embedding_dim, int32_t num_attention_heads,
  ...
  mlp = register_module("mlp",
                        std::make_shared<MultilayerPerceptron>(
...
auto Encoder::forward(torch::Tensor x, bool output_attention)
    -> std::tuple<torch::Tensor, std::optional<torch::Tensor>> {
...
Model::Model(Config config) : config(config) {
  ...
  embedding = register_module(
      "embedding",
      std::make_shared<Embedding>(num_cls_tokens, feature_width, feature_height,
